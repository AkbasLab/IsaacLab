...
constexpr TI GOAL_DELTA_DIM = 3; // goal_x - pos_x, goal_y - pos_y, goal_z - pos_z
#ifdef RL_TOOLS_ACTION_HISTORY
static_assert(ACTOR_TYPE::SPEC::INPUT_DIM == (18 + ACTION_HISTORY_LENGTH * ACTOR_TYPE::SPEC::OUTPUT_DIM + GOAL_DELTA_DIM));
#else
static_assert(ACTOR_TYPE::SPEC::INPUT_DIM == (18 + GOAL_DELTA_DIM));
#endif
...
void rl_tools_control(float* state, float* goal_delta, float* actions){
...
#endif
    // Append goal delta (dx, dy, dz) at positions 146, 147, 148
    constexpr TI GOAL_DELTA_START = 18 + ACTION_HISTORY_LENGTH * ACTOR_TYPE::SPEC::OUTPUT_DIM;
    rlt::set(input, 0, GOAL_DELTA_START + 0, goal_delta[0]);
    rlt::set(input, 0, GOAL_DELTA_START + 1, goal_delta[1]);
    rlt::set(input, 0, GOAL_DELTA_START + 2, goal_delta[2]);
